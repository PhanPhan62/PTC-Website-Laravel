        ...
        <div class="tables-wrapper ">  
            <div class="col-mr-3"> 
                <nav aria-label="breadcrumb">
                    ...
                    <a href="{{ url('/size/create') }}" class="main-btn active-btn-outline rounded-md btn-hover" style="padding: 10px 25px; margin-bottom: 5px;">Add</a>
                    </div>
                </div>
            <!-- end col -->
                                ...
                                <tbody>
                                    @foreach ($size as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action" >
                                                    <a style="cursor: pointer" href="{{ url('/size/show/' . $item->id) }}" class="text-dark mr-10">
                                                        <i class="lni lni-eye" title="View"></i>
                                                    </a>
                                                    <a style="cursor: pointer" href="{{route('edit.size', $item->id)}}" class="text-info  mr-10">
                                                        <i class="lni lni-write" title="Edit"></i>
                                                    </a>
                                                    <form method="POST" action="{{ route('destroy.size', $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                        {{ method_field('DELETE') }}
                                                        {{ csrf_field() }}
